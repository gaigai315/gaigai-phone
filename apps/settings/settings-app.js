...
// 获取默认提示词模板
getDefaultPrompt() {
    return this.settings.promptTemplate || `# 📱 虚拟手机 - 微信互动提示词

## 核心规则
当用户消息以 **[📱手机]** 开头时，表示用户通过手机微信给你发消息。

你需要：
1. 用JSON格式回复（见下方格式）
2. 可以发送**1-3条**消息（模拟真人打字习惯）
3. 消息间隔合理（timestamp字段）
4. 保持角色性格和说话风格
...
1. **只在有 [📱手机] 前缀时才用JSON格式**，否则正常回复
2. **多条消息更自然**（模拟真人边想边打字）
3. **时间戳可以用**："刚刚"、"10:23"、"1分钟前"
4. **可以用表情符号**：😊😭🤔等
5. **保持角色设定**：说话语气、性格特点要一致
...
| from | ✅ | 发送者名称 | "{{char}}" 或角色名 |
| message | ⭕ | 单条消息内容 | "在呢，怎么了？" |
| messages | ⭕ | 多条消息数组（推荐） | 见上方示例 |
| timestamp | ✅ | 时间戳 | "刚刚" / "10:23" |

**注意**：message 和 messages 二选一，推荐用 messages`;
}
